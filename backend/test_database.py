...
class DatabaseUrlTest(unittest.TestCase):
    ...
    def test_get_database_url_normalizes_postgres_url(self) -> None:
        with patch.dict(
            os.environ,
    ...
    def test_get_database_url_preserves_provider_query_parameters(self) -> None:
        with patch.dict(
            os.environ,
            {"DATABASE_URL": "postgresql://user:pass@db.example.com/danglish?sslmode=require"},
            clear=True,
        ):
            self.assertEqual(
                get_database_url(),
                "postgresql+psycopg://user:pass@db.example.com/danglish?sslmode=require",
            )
