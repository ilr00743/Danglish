...
def ingest_channels(
    ...
    for channel_id in channel_ids:
        try:
            ...
            print(f"[WARN] Skipping channel {channel_id}: {exc}", flush=True)
            skipped += 1
            continue
        ...
        for video in iter_channel_videos(youtube, uploads_playlist_id):
            ...
            try:
                ...
                print(f"[WARN] API error for {video_id}: {exc}", flush=True)
                skipped += 1
                continue
            ...
            if not transcript:
                ...
                print(f"[SKIP] No manually added Danish transcript for {video_id} - {title}", flush=True)
                skipped += 1
                continue
            ...
            processed += 1
            print(f"[OK] Indexed {video_id} - {title}", flush=True)
